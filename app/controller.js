...
function Controller()
{
	...
	this.sizeWindow = function()
	{
		...
		//var newImgWidth = Math.floor(width/NUM);
                if(width<340)
                {    
                    var newImgWidth = Math.floor(width/1);
                    var newImgHeight = Math.floor(newImgWidth/640 * 480);
                }
                else
                {
                    var newImgWidth = Math.floor(width/2);
                    var newImgHeight = Math.floor(newImgWidth/640 * 480);
                }    
		...
		$('.multiview_cam').width(newImgWidth);
                //$('.multiview_cam').width('100%');
    
		// Resize singleview images.
		var SINGLE_SCALE = 0.60;
                $('.singleview_cam img').width('100%');
		$('.singleview_cam img').height('100%');
                //$('.singleview_cam img').width(Math.floor(width * SINGLE_SCALE));
		//$('.singleview_cam img').height(Math.floor((width * SINGLE_SCALE)/640 * 480));

		//$('div.singleview_stats').width(Math.floor(width * (1.0 - SINGLE_SCALE)));
	};
};
